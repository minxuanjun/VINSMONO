...
void FeatureTracker::readImage(const cv::Mat &_img, double _cur_time) {
  ...
  if (!cur_pts.empty()) {
    ...
    status.clear();
    err.clear();
    vector<cv::Point2f> cur_pts_inverse = cur_pts;
    if (!forw_pts.empty()) {
    ...
    //          noArray(), Size(processor_config.patch_size,
    //          processor_config.patch_size), processor_config.pyramid_levels,
  ...
  if (PUB_THIS_FRAME) {
    ...
    if (n_max_cnt > 0) {
      ...
      cv::goodFeaturesToTrack(forw_img, n_pts, MAX_CNT - forw_pts.size(), 0.1,
                              MIN_DIST, mask);
    } else
      n_pts.clear();
